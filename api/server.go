...
type SafeExchangeConfig struct {
	...
	ExchangeType          string `json:"exchange_type"` // "binance", "bybit", "okx", "hyperliquid", "aster"
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if exchangeCfg == nil {
		...
		// Query actual balance if trader created successfully
		if createErr != nil {
			logger.Infof("⚠️ Failed to create temp trader: %v, using user input for initial balance", createErr)
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	enableFeedback := existingTrader.EnableFeedback // Keep original value
	if req.EnableFeedback != nil {
		enableFeedback = *req.EnableFeedback
	}
...
func (s *Server) handleUpdateExchangeConfigs(c *gin.Context) {
	...
	for exchangeID, exchangeData := range req.Exchanges {
		err := s.store.Exchange().Update(userID, exchangeID, exchangeData.Enabled, exchangeData.APIKey, exchangeData.SecretKey, exchangeData.Passphrase, exchangeData.Testnet, exchangeData.HyperliquidWalletAddr, exchangeData.AsterUser, exchangeData.AsterSigner, exchangeData.AsterPrivateKey)
		if err != nil {
...
type CreateExchangeRequest struct {
	ExchangeType          string `json:"exchange_type" binding:"required"` // "binance", "bybit", "okx", "hyperliquid", "aster"
	AccountName           string `json:"account_name"`                     // User-defined account name
	Enabled               bool   `json:"enabled"`
	APIKey                string `json:"api_key"`
	SecretKey             string `json:"secret_key"`
...
func (s *Server) handleKlines(c *gin.Context) {
	...
	case "hyperliquid", "hyperliquid-xyz", "xyz":
		...
		klines, err = market.GetKlinesHyperliquid(symbol, interval, limit)
		if err != nil {
	...
	default:
		...
		symbol = market.Normalize(symbol)
		klines, err = market.GetKlinesCoinank(symbol, interval, exchange, limit)
		if err != nil {
