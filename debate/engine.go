...
func (e *DebateEngine) runDebate(session *store.DebateSessionWithDetails, strategyConfig *store.StrategyConfig) {
	...
	baseSystemPrompt := strategyEngine.BuildSystemPromptWithContext(1000.0, ctx)
...
func (e *DebateEngine) buildMarketContext(session *store.DebateSessionWithDetails, strategyEngine *decision.StrategyEngine) (*decision.Context, error) {
	...
	ctx := &decision.Context{
		...
		PromptVariant:  session.PromptTemplate, // Use template as variant ID for debate
...
func (e *DebateEngine) collectVotes(session *store.DebateSessionWithDetails, strategyEngine *decision.StrategyEngine, allMessages []*store.DebateMessage) ([]*store.DebateVote, error) {
	...
	baseSystemPrompt := strategyEngine.BuildSystemPromptWithContext(1000.0, &decision.Context{})
