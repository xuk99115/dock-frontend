...
func (s *ExchangeStore) migrateToMultiAccount() error {
	...
	err := s.db.QueryRow(`
		...
		WHERE exchange_type = '' AND id IN ('binance', 'bybit', 'okx', 'bitget', 'hyperliquid', 'aster')
	`).Scan(&count)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.id, &r.userID, &r.name, &r.typ, &r.enabled,
			...
			&r.hyperliquidWalletAddr, &r.asterUser, &r.asterSigner, &r.asterPrivateKey); err != nil {
			return err
		}
...
func (s *ExchangeStore) Create(userID, exchangeType, accountName string, enabled bool,
	...
	hyperliquidWalletAddr, asterUser, asterSigner, asterPrivateKey string) (string, error) {
...
func (s *ExchangeStore) CreateLegacy(userID, id, name, typ string, enabled bool, apiKey, secretKey string, testnet bool,
	...
	if id == "binance" || id == "bybit" || id == "okx" || id == "bitget" || id == "hyperliquid" || id == "aster" {
