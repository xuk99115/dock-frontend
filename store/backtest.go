...
func (s *BacktestStore) initTables() error {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS backtest_prompt_variants (
			...
			prompt_role_definition TEXT DEFAULT '',
			prompt_trading_frequency TEXT DEFAULT '',
			prompt_entry_standards TEXT DEFAULT '',
			prompt_decision_process TEXT DEFAULT '',
...
func (s *BacktestStore) SaveConfig(runID, userID, promptTemplate, customPrompt, provider, model string, override bool, configJSON []byte) error {
	...
	_, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(run_id) DO NOTHING
	`, runID, userID, configJSON, promptTemplate, customPrompt, override, provider, model, now, now)
	if err != nil {
		return err
	}
	...
	_, err = s.db.Exec(`
		...
		WHERE run_id = ?
	`, userID, configJSON, promptTemplate, customPrompt, override, provider, model, runID)
	return err
}
